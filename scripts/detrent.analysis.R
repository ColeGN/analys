...
for (i in seq_along(variables)) {
  ...
  png(sprintf("output/plots/%s_linear_trend.png", var_name), 
  ...
  cat(sprintf("  ✅ Saved: output/plots/%s_linear_trend.png\n\n", var_name))
}
...
for (i in seq_along(variables)) {
  ...
  png(sprintf("output/plots/%s_moving_average.png", var_name), 
      width=1200, height=600, res=120)
  plot(y, type="l", lwd=2, col="blue",
  ...
  cat(sprintf("  ✅ Saved: output/plots/%s_moving_average.png\n\n", var_name))
}
...
write.csv(results, "output/results/forecast_evaluation.csv", row.names=FALSE)
cat("\n✅ Results saved to: output/results/forecast_evaluation.csv\n")
...
cat("   output/plots/     - All visualizations\n")
cat("   output/results/   - Forecast evaluation CSV\n\n")
